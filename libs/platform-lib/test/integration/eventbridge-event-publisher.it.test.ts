...
const BUS = 'pilot-eb-it';
const QUEUE = 'pilot-eb-it-q';
...
test('test_eventBridgePublisher_deliversEventWithEnvelopeToBus', async () => {
  ...
  await eb.send(new PutRuleCommand({ Name: 'all', EventBusName: BUS, EventPattern: JSON.stringify({ source: ['playfusion.pilot'] }) }));
