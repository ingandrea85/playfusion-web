...
describe('entitlements', () => {
  it('FREE is the restricted solo plan', () => {
    expect(entitlements('FREE')).toMatchObject({ maxSeats: 1, canInviteMembers: false, maxActiveEvents: 1, hasBrand: false, hasAnnouncements: false, hasPayments: false })
  })
  it('PRO unlocks the team (5 seats, unlimited events, modules)', () => {
    expect(entitlements('PRO')).toMatchObject({ maxSeats: 5, canInviteMembers: true, maxActiveEvents: null, hasBrand: true, hasAnnouncements: true, hasPayments: true, hasBusinessFeatures: false })
  })
