...
/** List rows feed the E1 dashboard and E3 landing. `organizationId` is part of the read
 *  contract (S18): the public portal resolves the tenant brand from it. `playbook` is always
 *  present (defaulted for pre-S6 rows). */
export type EventSummary = Omit<SportEvent, 'playbook'> & { playbook: Playbook };
export type EventDetail = EventSummary;

const strip = ({ playbook, ...rest }: SportEvent): EventDetail =>
