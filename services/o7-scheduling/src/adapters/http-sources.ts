...
export class HttpEventSource implements EventSource {
  ...
  async get(sportEventId: string): Promise<EventView | undefined> {
    ...
    return { sportEventId: e.sportEventId, dates: e.dates, categorie: e.categorie ?? [], gironi: e.gironi, sport: e.sport, tieBreak: e.tieBreak, sportProfile: e.sportProfile, format: e.format };
  }
}

/** Reads o5 confirmed registrations and buckets the team label by categoria. The label is the
 *  real `teamName` when set (S14 / PB-2 direct roster) else the `participantRef` (PB-1). */
