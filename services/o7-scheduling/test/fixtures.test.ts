import { test, expect } from 'vitest';
import { buildFixtures } from '../src/fixtures.js';
import type { FixtureCategory, ScheduleConfig } from '../src/domain.js';

const config: ScheduleConfig = { fields: ['Campo A', 'Campo B'], periods: 2, periodMinutes: 20, breakMinutes: 10, dailyStart: '09:00', slotsPerDay: 8, groupsCount: 1, legs: 'SINGLE' };

test('test_buildFixtures_splitsTeamsIntoGroupsAsSingleLegRoundRobin', () => {
  const cats: FixtureCategory[] = [{ id: 'U10', name: 'U10', groupsCount: 2, legs: 'SINGLE', teams: ['A', 'B', 'C', 'D'] }];
  const m = buildFixtures('evt-1', '2026-08-29', '2026-08-30', config, cats);
  expect(m).toHaveLength(2); // group A: A-C, group B: B-D
  expect(m[0]).toMatchObject({ home: 'A', away: 'C', groupLabel: 'Girone A', field: 'Campo A', time: '09:00', day: '2026-08-29' });
  expect(m[1]).toMatchObject({ home: 'B', away: 'D', groupLabel: 'Girone B', field: 'Campo B', time: '09:00' });
});

test('test_buildFixtures_doublesMatchesForHomeAway', () => {
  const cats: FixtureCategory[] = [{ id: 'U10', name: 'U10', groupsCount: 1, legs: 'HOME_AWAY', teams: ['A', 'B', 'C'] }];
  const m = buildFixtures('evt-1', '2026-08-29', '2026-08-30', config, cats);
  expect(m).toHaveLength(6);
  expect(m.filter((x) => x.home === 'B' && x.away === 'A')).toHaveLength(1);
});

test('test_buildFixtures_placesFieldThenSlot', () => {
  const cats: FixtureCategory[] = [{ id: 'U10', name: 'U10', groupsCount: 1, legs: 'SINGLE', teams: ['A', 'B', 'C', 'D'] }];
  const m = buildFixtures('evt-1', '2026-08-29', '2026-08-30', config, cats);
  expect(m).toHaveLength(6);
  // slotMinutes = 2*20 + 10 = 50; 2 fields → the 3rd match wraps to the next slot on Campo A.
  expect(m[2]).toMatchObject({ field: 'Campo A', time: '09:50' });
});

test('test_buildFixtures_isDeterministicWithSmIds', () => {
  const cats: FixtureCategory[] = [{ id: 'U10', name: 'U10', groupsCount: 1, legs: 'SINGLE', teams: ['A', 'B'] }];
  const m = buildFixtures('evt-1', '2026-08-29', '2026-08-29', config, cats);
  expect(m).toHaveLength(1);
  expect(m[0]!.id).toBe('sm-1');
  expect(m[0]).toMatchObject({ sportEventId: 'evt-1', categoryId: 'U10', home: 'A', away: 'B', day: '2026-08-29' });
});

test('test_buildFixtures_emptyWhenNoTeams', () => {
  const cats: FixtureCategory[] = [{ id: 'U10', name: 'U10', groupsCount: 1, legs: 'SINGLE', teams: [] }];
  expect(buildFixtures('evt-1', '2026-08-29', '2026-08-30', config, cats)).toHaveLength(0);
});
