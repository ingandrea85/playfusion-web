...
import { WindowClosedError, DoubleApplyError, UnknownParticipantError } from '../domain/errors.js';
...
export const applyRegistration = (d: Deps) => async (cmd: Cmd) => {
  ...
  if (!(await d.participants.exists(cmd.participantRef))) throw new UnknownParticipantError(cmd.participantRef);
