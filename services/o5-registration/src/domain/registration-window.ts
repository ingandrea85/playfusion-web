...
/** `enrollToken` (coach enrollment): a shared coach magic-link minted when the organizer
 *  opens the window and handed out by the organizer. Persisted so the same link is stable
 *  across re-opens; exposed only to the organizer (never in the public window read). */
export type RegistrationWindow = { sportEventId: string; state: WindowState; capacities?: Record<string, number>; enrollToken?: string };
